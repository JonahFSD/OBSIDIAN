...
Everything else on disk — `REFERENCE.md`, the `.github/prompts/` workflows, vault notes —
is a pointer to, or a projection of, what is written here. If any other document disagrees
with this one, **this document wins and the other is stale.**

- **Workspace root** = the git repository root. On this machine that is `~/CODE`
  (`/Users/jonahelliott/CODE`). All paths below are relative to it.
...
- **Built:** 2026-06-12, fresh from the Obsidian Memory System reference. See `PROVENANCE.md`.
...
The daily logs are the journal; the vault is durable storage; retro is the compaction pass
that merges journal into durable storage; git is the ledger underneath all of it.
...
| **Working** | Active Context, action ledger, today's briefing + logs | Hours–days | Workspace root + `YYYY-MM-DD/` |
| **Durable** | Vault notes, Meta-Cognition, Retrieval Strategies | Sessions–permanent | `KB/` |

Routing rule of thumb: **matters today → briefing/logs · matters across sessions → vault ·
changes how Charlie operates → `KB/Meta-Cognition.md` · changes how Charlie searches →
`KB/Retrieval Strategies.md` · unclear → `KB/Inbox/`.**
...
```
~/CODE/                              ← workspace root = git repo root = transient operating surface
├── .git/                            Local-only repository. No remote. The ledger.
├── .gitignore                       Excludes KB/.obsidian/, KB/.trash/, .DS_Store. App state is not memory.
│
...
│
├── profile.md                       PINNED. Stable profile of Jonah + Charlie: preferences, tone, context.
...
│
├── tools/
│   ├── lint_vault.py                Schema linter. Python-3 stdlib only (no pip installs).
│   ├── lint.sh                      The ONLY sanctioned invocation. Wraps python3 lint_vault.py,
│   │                                propagates exit code. Never retype the raw python line.
│   └── retro_status.py              Staleness helper: days since newest retro. Used by warmup + hook.
│
├── .github/prompts/
│   ├── Charlie.agent.md             Loader/pointer → CHARLIE.md.
│   ├── warmup.prompt.md             Warmup workflow + the staleness ratchet.
│   ├── remember.prompt.md           Durable-capture workflow.
│   ├── retro.prompt.md              The transactional retro protocol.
│   └── thermonuclear-code-quality-review.prompt.md   Heavy review mode; load on explicit request only.
│
├── .claude/settings.json            Advisory retro Stop hook (defense-in-depth; ratchet is primary).
│
├── YYYY-MM-DD/                      Daily folders, kept at root ~14 days, then archived.
...
│
└── KB/                             ← THE VAULT (durable memory). Also an Obsidian vault.
...
vault notes (**canonical-note-first:** find and update the existing note in place; never
duplicate), detects supersession (old note marked, never silently contradicted), promotes
repeatedly load-bearing material toward the pinned files, demotes/evicts pinned material that
is over budget, and commits. Daily logs are immutable after compaction; the distilled layer is
always re-derivable from them.
...
1. **Folder ↔ type.** A note's `type` must match its `Notes/` subfolder. `Inbox/` and
   ...
   `status: archived`. The two root hub files (`Meta-Cognition.md`, `Retrieval Strategies.md`)
   are exempt from the folder law and carry `type: concept`.
...
**Linter contract.** `tools/lint.sh` walks `KB/` recursively and FAILS on: missing/invalid
...
manually anytime. It depends only on Python 3 standard library (no pip).
...
to a vault note and a one-line link stays behind. The Health block reports the current size
every retro.
...
**Mandatory first step — the staleness ratchet:** run `python3 tools/retro_status.py` (or
`tools/retro_status.py` directly) to compute days since the newest `KB/Notes/Retro/*.md`. If
**> 1 day**, open with: *"Retro is N days overdue — running it now before new work (say
'defer' to override)"* and default to running retro. Then: read `Active Context.md` → follow
≤ 3 links → `profile.md` → open ledger items → today's briefing if present. Return: current
focus, top open threads, next likely action.
...
update it in place → ask before writing ambiguous memory → anything unclear goes to `KB/Inbox/`.
...
- **a. First action.** Create `KB/Notes/Retro/YYYY-MM-DD retro.md` containing a checklist of
...
- **h. Archive.** Move daily folders older than 14 days → `KB/Archive/daily/` (move, not delete).
...
- **I2 — Compaction by default.** Retro is the default exit, not the exception. Enforced by the
  warmup staleness ratchet (§7.2) and, on Claude Code, an advisory Stop hook
  (`.claude/settings.json`). Skips are never silent — each writes `retro deferred YYYY-MM-DD`
  to `action-ledger.md`.
- **I3 — Schema.** Every vault note conforms, machine-checked by `tools/lint.sh`. The linter
...
- **I9 — Explicit growth triggers.** Deferred components have written entry triggers, kept in
  `KB/Meta-Cognition.md`. A component earns its place only when the failure mode it prevents
  has occurred **twice.**
...
|---|---|---|
| Vault linter | `tools/lint.sh` | Only sanctioned form. Never retype the raw `python3 lint_vault.py` line. |
| Retro staleness | `python3 tools/retro_status.py` | Days since newest retro; drives the warmup ratchet. |
| Git ledger | standard git, local | Commit at every retro. `git log -p "Active Context.md"` = history of Charlie's understanding. |
| Obsidian (app) | manual | Viewer/editor only; its state dir is gitignored; close it during bulk file moves to avoid lock issues. |
